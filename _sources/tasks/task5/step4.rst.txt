...
#. Let's now add a new test case by clicking on the :guilabel:`+` button. A pop-up will appear, enter the information for the test case as detailed in the following image. Click on the :guilabel:`SUBMIT` button when you are done.

#. Click on the :guilabel:`Open` link to display the test case details. A pop-up will appear, click on the :guilabel:`Configure Automation` button.
...
#. The execution status will show `COMPLETED` for a successful execution. If the execution status is anything other than `COMPLETED` , please reach out to your session speakers.

#. Refresh the page to see the execution details move to the `Run History` section. 
